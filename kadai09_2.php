...
if( ! $instance -> connect_error){
    ...
    $sql = "UPDATE OLDPRODUCT SET PNAME =?, CATEGORY =?,PRICE =?  WHERE PRODUCT_NO =?";
    
    if($stmt = $instance -> prepare($sql)){
      $stmt->bind_param("ssis",$postdata["pname"],$postdata["category"],$postdata["price"],$postdata["product_no"]); 
      $stmt->execute();
      //9
      ...
      //更新失敗ロールバック処理
      $instance -> rollback();
      $_SESSION["errmessage"] = "商品情報更新できませんでした。";
      header('Location: kadai09_1.php');
      }
      $stmt -> close();
      }   
